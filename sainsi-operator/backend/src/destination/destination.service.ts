...
import { SafetyReport, SafetyReportDocument } from '../schemas/safety-report.schema';
import { WeatherCache, WeatherCacheDocument } from '../schemas/weather-cache.schema';
...
export class DestinationService {
  constructor(
    ...
    @InjectModel(SafetyReport.name) private safetyReportModel: Model<SafetyReportDocument>,
    @InjectModel(WeatherCache.name) private weatherCacheModel: Model<WeatherCacheDocument>,
  ...
  async findOne(id: string): Promise<Destination | null> {
    return this.destinationModel.findById(id).populate('similarDestinations').exec();
  }

  async getExperiences(destinationId: string): Promise<Experience[]> {
    return this.experienceModel.find({ destination: destinationId }).populate('operator').exec();
  }

  async getSafetyReport(destinationId: string): Promise<SafetyReport | null> {
    return this.safetyReportModel.findOne({ destination: destinationId }).exec();
  }
  ...
  async getWeather(destinationId: string): Promise<WeatherCache | null> {
    ...
    return this.weatherCacheModel.findOne({ destination: destinationId }).exec();
  }

  async getOperators(destinationId: string): Promise<Operator[]> {
    // Find operators that have experiences in this destination
    const experiences = await this.experienceModel.find({ destination: destinationId }).distinct('operator').exec();
    return this.operatorModel.find({ _id: { $in: experiences } }).exec();
  }

  async getQuestions(destinationId: string): Promise<Question[]> {
    return this.questionModel.find({ destination: destinationId }).populate('user').populate('answers.author').sort({ upvotes: -1 }).exec();
  }
  ...
  async getReviews(destinationId: string): Promise<Review[]> {
    ...
    return this.reviewModel.find({ operator: { $in: operatorIds } }).populate('reviewer').exec();
  }
}
